...
describe('ApiService', () => {
  ...
  it('should make a GET request to fetch user data', () => {
    service.getUser('saranshh').subscribe((response) => {
      expect(response.body).toEqual(mockUser);
    });

    const req = httpMock.expectOne('https://api.github.com/users/saranshh');
    expect(req.request.method).toBe('GET');
    req.flush(mockUser);
  });
  
  it('should make a GET request to fetch user repositories', () => {
    service.getRepos('saranshh', 1, 10).subscribe((response) => {
      expect(response.body).toEqual(mockRepoData);
    });

    const req = httpMock.expectOne(
      'https://api.github.com/users/saranshh/repos?sort=updated&page=1&per_page=10'
    );
    expect(req.request.method).toBe('GET');
